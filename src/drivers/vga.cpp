...
void VideoGraphicsArray::WriteRegisters(uint8_t *registers)
{
   ...
   crtcDataPort.Write(crtcDataPort.Read() & ~0x80);
   /* register 0-7 of crtc are write protected by protect bit
      (bit 7 of index 0x11) so we must clear it to write to registers */
...
void VideoGraphicsArray::PutPixel(int32_t x, int32_t y, uint8_t colorIndex)
{
   ...
   uint8_t* pPixelAddress = GetFrameBufferSegment() + 320*y+x;
   *pPixelAddress = colorIndex;
}
